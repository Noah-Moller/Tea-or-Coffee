...
type PopularResponse = {
  items: { drink: string; count: number }[];
};
...
let cart: CartItem[] = [];
let popularItems: Map<string, number> = new Map();
...
function renderDrinkCard(drink: string): HTMLElement {
...
function renderMenu(menu: string[]): void {
...
function openItemModal(drink: string, existingInstructions?: string, cartIndex?: number): void {
  const modal = document.getElementById("item-modal");
  const title = document.getElementById("item-modal-title");
  const drinkInput = document.getElementById("item-drink") as HTMLInputElement;
  const instructionsInput = document.getElementById("item-instructions") as HTMLTextAreaElement;
